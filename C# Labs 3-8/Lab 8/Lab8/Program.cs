...
namespace Lab8
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (i = 0; i < count; i++)
            {
                order[i].DoNothing += order[i].Down;
                order[i].DoNothing += delegate ()
                {
                    Console.WriteLine("Столько материала нет в наличии. Размер заказа был уменьшен.");
                };
                order[i].DoSomething();
                order[i].Scost(order[i].Material, order[i].Mass);
                order[i].Scost(order[i].Material, order[i].Mass, order[i].Count);
                ColorPrint(i, delegate ()
                {
                    order[i].Output();
                });
            }
        ...
        private static void ColorPrint(int i, Production.ShowColorText show)
        {
            if (i % 2 == 0)
            {
